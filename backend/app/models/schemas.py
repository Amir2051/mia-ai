...
class ShopSession(Base):
    ...
    shop: Mapped['Shop'] = relationship(back_populates='sessions')
...
class ProductImport(Base):
    ...
    shop: Mapped['Shop'] = relationship(back_populates='imports')
...
class AppSetting(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    shop_id: Mapped[Optional[int]] = mapped_column(ForeignKey('shops.id', ondelete='CASCADE'), nullable=True)
    key: Mapped[str] = mapped_column(String(255))
    value_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, onupdate=utcnow)

    shop: Mapped[Optional['Shop']] = relationship(back_populates='settings')
