...
class CsvImportService(ImportService):
    ...
    def normalize_duplicate_action(value: Optional[str]) -> str:
        ...
        if action not in VALID_DUPLICATE_ACTIONS:
            raise ValueError(
                "duplicate_action must be one of: skip, update, create, fail"
            )
        return action
    ...
    async def find_duplicate_product(self, mapped: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for kind, query in queries:
            ...
            for product in products:
                ...
                variant_nodes = variants.get("nodes") or []
                skus = {
                    str(v.get("sku") or "").strip().casefold()
    ...
    async def _execute_csv(
        ...
        for item in validation["valid"]:
            processed += 1
            prepared = self._prepare_product(item["mapped"], default_status)
            ...
            try:
                duplicate = await self.find_duplicate_product(prepared)
                if duplicate:
                    ...
                    if duplicate_action == "skip":
                        ...
                        details.append({
                            ...
                            "shopify_product_id": duplicate_id,
                            "mapped": prepared,
                        })
                        if duplicate_id:
                    ...
                    if duplicate_action == "fail":
                        ...
                        details.append({
                            "row": item["row"],
                            "status": "failed",
                            "error": f"Duplicate product found by {match}",
                            "duplicate_match": match,
                            "shopify_product_id": duplicate_id,
                            "mapped": prepared,
                        })
                    ...
                    if duplicate_action == "update":
                        ...
                        result = await self.api_client.update_product(duplicate_id, prepared)
                        updated += 1
                        shopify_product_ids.append(duplicate_id)
                        details.append({
                            "row": item["row"],
                    ...
                    # create intentionally falls through and creates a new product.
        ...
        if validate_only:
            status, sync_status = "validated", "preview"
        elif failed:
            status, sync_status = "completed_with_errors", "completed"
        else:
            status, sync_status = "completed", "completed"
    ...
    async def create_import_from_csv(self, shop_id: int, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        record = ProductImport(
            ...
            tags=",".join(str(tag) for tag in payload.get("tags") or [] if str(tag).strip()),
